...
//check for errors
if(empty($_POST["currentpasswordowner"])){
    $errors .= $missingCurrentPassword;
}else{
    $currentPassword = $_POST["currentpasswordowner"];
    ...
    $sql = "SELECT password FROM teamownerusers WHERE user_id='$user_id'";
...
if($errors){
    ...
    $sql = "UPDATE teamownerusers SET password='$password' WHERE user_id='$user_id'";
    $result = mysqli_query($link, $sql);
    if(!$result){
