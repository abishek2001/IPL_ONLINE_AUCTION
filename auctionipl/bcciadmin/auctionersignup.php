...
//Get username
if(empty($_POST["usernameauct"])){
    $errors .= $missingUsername;
}else{
    $username = filter_var($_POST["usernameauct"], FILTER_SANITIZE_STRING);
}
//Get email
if(empty($_POST["emailauct"])){
    $errors .= $missingEmail;
}else{
    $email = filter_var($_POST["emailauct"], FILTER_SANITIZE_EMAIL);
...
//If username exists in the adminusers table print error
...
//If email exists in the adminusers table print error
...
//Insert user details and activation code in the adminusers table
$sql = "INSERT INTO adminusers (`username`, `email`, `password`) VALUES ('$username', '$email', '$password')";
$result = mysqli_query($link, $sql);
if(!$result){
    echo '<div class="alert alert-danger">There was an error inserting the auctioner users details in the database!</div>';
    exit;
}
else
{
       echo "<div class='alert alert-success'>Auctioner Account '$username' has been succesfully created</div>";
}
